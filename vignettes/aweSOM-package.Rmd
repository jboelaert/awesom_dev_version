...
The **circular barplot** displays the mean value for each of the variables, per cell
```{r }
aweSOMplot(som = ok.som, type = 'Circular', data = ok.data, 
           variables = c("Sepal.Length", "Sepal.Width",  
                         "Petal.Length", "Petal.Width"),  
           size = 400)
```

This can also be represented with standard barplots when the number of variables is small. 
```{r}
...
aweSOMdendrogram(clust = superclust_hclust, nclass = 2)
```
...
```{r, fig.width=7, fig.height=4 } 
aweSOMscreeplot(som = ok.som, method = 'hierarchical', 
